...
$(document).ready(function() {
    ...
    function loadClass(sessionSelectId, classDivId, subjectDivId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    // Show class name
                    $('#' + classDivId).html(`<strong>Class:</strong> ${response.class.name}`);

                    // Now fetch subjects for this class
                    $.ajax({
                        ...
                        success: function(subjectRes) {
                            if (subjectRes.success) {
                                let subjectList = "<strong>Subjects:</strong><ul>";
                                subjectRes.subjects.forEach(function(sub) {
                                    subjectList += `<li>${sub.sub_name}</li>`;
                                });
                                subjectList += "</ul>";
                                $('#' + subjectDivId).html(subjectList);
                            } else {
                                $('#' + subjectDivId).html('<span class="text-danger">No subjects found</span>');
                            }
                        }
                    });

                } else {
                    $('#' + classDivId).html('<span class="text-danger">Class not found</span>');
                    $('#' + subjectDivId).empty();
                }
            }
        });
    ...
    $('#comparison1, #comparison2').change(function() {
        ...
        if (val1 && val2 && val1 === val2) {
            alert('Both comparisons cannot be the same session.');
            $('#comparison2').val('');
            $('#class2-div').empty();
            $('#subject2-div').empty();
            return;
        ...
        if (this.id === 'comparison1') {
            loadClass('comparison1', 'class1-div', 'subject1-div');
        }

        if (this.id === 'comparison2') {
            loadClass('comparison2', 'class2-div', 'subject2-div');
        }
    });
});
</script> --}}
                        ...
                        success: function (subjectRes) {
                            ...
                            if (subjectRes.success && subjectRes.subjects.length > 0) {
                                ...
                                $.each(subjectRes.subjects, function (index, subject) {
                                    subjectSelect.append(`<option value="${subject.id}">${subject.sub_name}</option>`);
                                });
